...
public class GrafoNDNP {
	...
	public void grabarEntradaGrafo(String pathIn) {
		...
		try {
			...
			entrada.println(this.cantidadDeNodos + " " + this.cantidadDeAristas + " " + this.porcentajeAdyacencia + " " + this.gradoMaximo + " "
					+ this.gradoMinimo);

			for (int i = 0; i < cantidadDeNodos; i++) {
				for (int j = i + 1; j < cantidadDeNodos; j++) {
					if (this.matriz.getIJ(i, j))
						entrada.println(i + " " + j);
				}
			}
	...
	private boolean sePuedeColorear(int pos, int color) {
		...
		while (i < this.cantidadDeNodos) {
			if (nodos[i].getColor() == color) { //si el nodo a colorear es adyacente a un nodo ya pintado de ese
				if (esAdyacente(nodos[i], nodos[pos])) //color, no lo podr� colorear
					return false;
			}
			i++;
		}
	...
	private boolean esAdyacente(Nodo nodoUno, Nodo nodoDos) {
		return matriz.getIJ(nodoUno.getNumero(), nodoDos.getNumero()) == true;
	}
	...
	public void alterarOrdenNodos() {
		...
		for (int i = 0; i < cantidadDeNodos; i++) {
			pos = rnd.nextInt(aux);
			vectorAux[numeros[pos]] = new Nodo(nodos[i].getNumero(), 0, nodos[i].getGrado()); //cambio la pos del nodo, en un vector de nodos aux
	...
	public void ejecutarCaso(int codAlgortimo, int cantCorridas, String path) {
		...
		for (int i = 0; i < cantCorridas; i++) {
			selectorColoreoGrafo(codAlgortimo);
			cantColor[cantidadDeColores] += 1;
			if (cantidadDeColores < nroCromatico) {
				nroCromatico = cantidadDeColores;
				grafoColoreado = nodos;
			}
			if(nroCromatico<minColor){
				minColor=nroCromatico;
				minCorrida = i;
			}
			alterarOrdenNodos();
		}
		grabarResumenCaso(codAlgortimo, cantColor, nroCromatico, path, minCorrida);
	}

	public void grabarResumenCaso(int cod_algoritmo, int[] cantColor, int nroCromatico, String path, int minCorrida) {
		FileWriter fw = null;
		PrintWriter pw = null;
		try {
			...
			pw.println();
			pw.println("La mejor corrida fue la: "+minCorrida);
		} catch (IOException e) {
			e.printStackTrace();
		} finally {
			try {
